...
for itoy in xrange(0,ntoys) :
    ...
    unfold = RooUnfoldTUnfold(response, hToy_i[itoy], TUnfold.kRegModeCurvature)
...
unfoldUp = RooUnfoldTUnfold(response, hMeasUp, TUnfold.kRegModeCurvature)
unfoldDn = RooUnfoldTUnfold(response, hMeasDn, TUnfold.kRegModeCurvature)
...
hMeasCheckUp = TH1D("hMeasCheckUp", "measured check up", nbinsMeas, 0, nbinsMeas)
hMeasCheckDn = TH1D("hMeasCheckDn", "measured check down", nbinsMeas, 0, nbinsMeas)

lowedge = 399.
highedge = 1199.

for ibin in range(0, nbinsMeas):

    if hMeas.GetBinLowEdge(ibin+1) > lowedge and hMeas.GetBinLowEdge(ibin+1) < highedge:
...
print ""
print ' pvalue = ', pvalue, ' pvalueUP = ', pvalueUp, ' pvalueDN = ', pvalueDn
print ""
...
for ip in xrange(0,nPoints):
    ...
    print 'Point ' + str(ip) + ': logTau = ' + str(logTau) + ', x = ' + str(tmpX) + ', y = ' + str(tmpY)
...
c2.SaveAs("LCurve"+options.nbr+"_"+options.lepType+"_"+options.toy+"_"+options.type+".pdf")
