...
if options.lepType=="ele" and not options.closureTest:
    f_data1 = TFile("histfiles_80X/hists_Data_el.root")
    f_QCD1  = TFile("histfiles_80X/hists_Data_el_qcd.root")
elif not options.closureTest:
    f_data1 = TFile("histfiles_80X/hists_Data_mu.root")
    f_QCD1  = TFile("histfiles_80X/hists_Data_mu_qcd.root")
...
if not options.closureTest: 
    hMeas = f_data.Get("ptRecoTop"+options.nbr)
else :
...
if not options.closureTest: 
    ...
    hMeas_qcd                    = f_QCD.Get("ptRecoTop"+options.nbr)
...
if not options.closureTest: 
    ...
    hNorm_QCD_HT300to500   = f_QCD_HT300to500.Get("ptRecoTop"+options.nbr)
    ...
    hNorm_QCD_HT1000to1500.Scale(1207. * lum / 4980387.)
