...
void makePlots(TString DIR, TString DIRqcd, TString channel, TString var, TString region, bool useQCDMC = false, bool unBlind = false, bool usePost = false) {
  ...
  if (h_wjets == 0) cout << "NO WJETS!" << endl;
  TH1F* h_ttbar = (TH1F*) ttbar->hist();
  if (h_ttbar == 0) cout << "NO TTBAR SIGNAL!" << endl;
  TH1F* h_ttbar_nonSemiLep = (TH1F*) ttbar_nonSemiLep->hist();
  if (h_ttbar_nonSemiLep == 0) cout << "NO TTBAR OTHER!" << endl;
  TH1F* h_singletop = (TH1F*) singletop->hist();
  if (h_singletop == 0) cout << "NO SINGLETOP!" << endl;
  TH1F* h_data = (TH1F*) data->hist();
  if (h_data == 0) cout << "NO DATA!" << endl;
...
void makeTable(TString DIR, TString DIRqcd, TString channel, bool inSideband, bool useQCDMC, bool unBlind) {
  ...
  cout << "Data                 &            ";
  for (int ii = 0; ii < nhist; ii++){
    if (ii == 0 || unBlind || inSideband) cout << setw(7) << count_data[ii];
    else cout << "  N/A  ";
    if (ii == nhist - 1) cout << " \\\\ " << endl;
    else cout << "      &      ";
  }
